...
public class PortfolioMonthlySummaryService {
     ...
     * This method creates realistic monthly summaries with varying gains/losses
     * 
     * @return Number of summaries created
     */
    public int createDummyHistoricalData() {
        LocalDate now = LocalDate.now();
        int currentYear = now.getYear();
        int currentMonth = now.getMonthValue();
        
        // Starting from June 2024 to current month
        LocalDate startDate = LocalDate.of(2024, 6, 1);
        LocalDate endDate = LocalDate.of(currentYear, currentMonth, 1);
        
        // Starting portfolio value (realistic starting point)
        BigDecimal startingValue = new BigDecimal("50000.00");
        BigDecimal currentValue = startingValue;
        
        int createdCount = 0;
        
        // Create data from June 2024 to current month
        ...
        while (!currentDate.isAfter(endDate)) {
            ...
            // Calculate realistic monthly changes
            BigDecimal monthlyChange;
            BigDecimal monthlyChangePercent;
            
            if (currentDate.equals(startDate)) {
                // First month (June 2024) - no previous data
                monthlyChange = BigDecimal.ZERO;
                monthlyChangePercent = BigDecimal.ZERO;
            } else {
                // Generate realistic monthly gains/losses
                // Random monthly change between -8% and +12%
                double randomPercent = (Math.random() * 20) - 8; // -8 to +12
                monthlyChangePercent = new BigDecimal(String.valueOf(randomPercent));
                monthlyChange = currentValue.multiply(monthlyChangePercent)
                        .divide(new BigDecimal("100"), 2, RoundingMode.HALF_UP);
            }
            
            // Calculate new total value
            BigDecimal newTotalValue = currentValue.add(monthlyChange);
            
            // Create the monthly summary
            PortfolioMonthlySummary summary = new PortfolioMonthlySummary(
                    year, month, newTotalValue, monthlyChange, monthlyChangePercent
            );
            
            portfolioMonthlySummaryRepository.save(summary);
            createdCount++;
    ...
    /**
     * Create comprehensive dummy data (both monthly and daily)
     * 
     * @return Map with counts of created records
     */
    public java.util.Map<String, Integer> createComprehensiveDummyData() {
        java.util.Map<String, Integer> results = new java.util.HashMap<>();
        
        int monthlyCount = createDummyHistoricalData();
        
        results.put("monthlySummariesCreated", monthlyCount);
        results.put("dailyRecordsCreated", 0); // Removed daily data creation
